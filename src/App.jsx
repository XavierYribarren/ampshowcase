...
export default function App() {
  ...
  const [faustReady, setFaustReady] = useState(false);
  const libfaustWasmUrl = '/libfaust/libfaust-wasm.wasm';
  ...
  useEffect(() => {
    ...
    const loadFaust = async () => {
      try {

        // await new Promise((resolve, reject) => {
        //   const script = document.createElement('script');
        //   script.src = '/libfaust/libfaust-wasm.js';
        //   script.onload = resolve;
        //   script.onerror = reject;
        //   document.body.appendChild(script);
        // });
    const { LibFaust, FaustCompiler, FaustMonoDspGenerator } = window.Faust;

        const module = await instantiateFaustModuleFromFile(libfaustWasmUrl);
        ...
        console.error('Faust init failed:', err);
      }
    };
      ...
      <main className='main-wrap' style={{ fontFamily: 'sans-serif' }}>
        <div className='select'>
          ...
          <div className='file-play-stop'>
            ...
            <label htmlFor='audioFileInput' className='file-input-label'>
              <MdFindReplace size={24} /> Select Audio File
            </label>
